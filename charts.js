...
function createDemoData() {
    ...
    // Dados para todos os anos
    productionData = {
        labels: [...months.map(m => '2023 - ' + m), ...months.map(m => '2024 - ' + m)],
        datasets: [
            {
                label: 'Banana',
                data: [...production2023Data.datasets[0].data, ...production2024Data.datasets[0].data],
                borderColor: '#FFC107',
                backgroundColor: 'rgba(255, 193, 7, 0.5)',
                borderWidth: 2,
                fill: false,
            ...
            {
                label: 'Goiaba',
                data: [...production2023Data.datasets[1].data, ...production2024Data.datasets[1].data],
                borderColor: '#4CAF50',
                backgroundColor: 'rgba(76, 175, 80, 0.5)',
                borderWidth: 2,
                fill: false,
            ...
            {
                label: 'Abacaxi',
                data: [...new Array(12).fill(0), ...production2024Data.datasets[2].data],
                borderColor: '#2196F3',
                backgroundColor: 'rgba(33, 150, 243, 0.5)',
                borderWidth: 2,
                fill: false,
...
function loadStaticData() {
    ...
    // Criar dados para "Todos os Anos"
    const allLabels = [];
    const allDatasets = [];
    
    // Adicionar labels de ambos os anos
    months.forEach(month => {
        allLabels.push(`2023 - ${month}`);
    });
    
    months.forEach(month => {
        allLabels.push(`2024 - ${month}`);
    });
    
    // Combinar datasets de 2023 e 2024
    const allCultures = new Set();
    data2023.datasets.forEach(ds => allCultures.add(ds.label));
    data2024.datasets.forEach(ds => allCultures.add(ds.label));
    
    allCultures.forEach(culture => {
        const dataset2023 = data2023.datasets.find(ds => ds.label === culture);
        const dataset2024 = data2024.datasets.find(ds => ds.label === culture);
        
        const combinedData = [];
        
        // Adicionar dados de 2023, ou zeros se a cultura não existir
        if (dataset2023) {
            combinedData.push(...dataset2023.data);
        } else {
            combinedData.push(...new Array(12).fill(0));
        }
        
        // Adicionar dados de 2024, ou zeros se a cultura não existir
        if (dataset2024) {
            combinedData.push(...dataset2024.data);
        } else {
            combinedData.push(...new Array(12).fill(0));
        }
        
        // Usar as cores e estilos do dataset existente
        const sourceDataset = dataset2023 || dataset2024;
        
        allDatasets.push({
            label: culture,
            data: combinedData,
            borderColor: sourceDataset.borderColor,
            backgroundColor: sourceDataset.backgroundColor,
            borderWidth: 2,
            fill: false,
            tension: 0.4
        });
...
function initChartFilters() {
    ...
    filterButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            if (productionChart && productionDataByYear && productionDataByYear[year]) {
                ...
                console.log('Gráfico atualizado para', year, 'com', productionDataByYear[year].datasets.length, 'culturas');
            } else {
                console.error('Gráfico ou dados do ano', year, 'não disponíveis');
            }
        });
    });
    
    // Acionar o botão "Todos os Anos" inicialmente
...
function addChartButtons(chartContainer, chart, filename) {
    ...
    chartActions.style.position = 'absolute';
    chartActions.style.top = '5px';
    chartActions.style.right = '5px';
    chartActions.style.zIndex = '100';
    ...
    expandBtn.style.margin = '0 3px';
    expandBtn.style.cursor = 'pointer';
    expandBtn.style.boxShadow = '0 2px 5px rgba(0,0,0,0.2)';
    ...
    downloadBtn.style.margin = '0 3px';
...
function toggleFullscreen(element, buttonElement, chart) {
    ...
    if (isFullscreen) {
        ...
        fullscreenCanvas.style.height = '80%';
        
        modal.appendChild(closeButton);
        modal.appendChild(fullscreenCanvas);
